...
export function useGameState(walletAddress: string | null) {
  ...
  useEffect(() => {
    setPlayer((prev) => ({
      ...
      address: walletAddress,
      deck:
        walletAddress && prev.deck.length === 0
          ? generateStarterDeck()
          : prev.deck,
    }));
  }, [walletAddress]);
  ...
  const startRun = useCallback(() => {
    ...
    const currentDeck =
      player.deck.length > 0 ? player.deck : generateStarterDeck();
    ...
    setGameState({
      ...
      selectedCards: [],
      battleLog: [
        { type: "attack", message: `Stage 1: ${boss.name} appears!` },
      ],
      runScore: 0,
    });
  ...
  const executeBattle = useCallback(() => {
    if (gameState.selectedCards.length !== 3 || !gameState.currentBoss)
      return null;
  ...
  const submitWinOnChain = useCallback(
    async (wallet: any, playerObjectId: string) => {
      ...
      try {
        ...
        const result = await winStageAndFetchNFT(
          wallet,
          playerObjectId,
          stage,
          level,
        ...
        try {
          const objRes: any = await suiClient.getObject({
            id: nftId,
          ...
          const image_url =
            fields?.image_url ?? fields?.imageUrl ?? fields?.image ?? "";
          ...
          try {
            await saveNFT(bossNFT);
          } catch (_) {}
          setPlayer((prev) => ({
            ...prev,
            bossNFTs: [...prev.bossNFTs, bossNFT],
          }));
          setPendingNFT(null);
        } catch (err) {
          return result;
  ...
  const nextStage = useCallback(() => {
    ...
    setGameState((prev) => ({
      ...
      battleLog: [
        {
          type: "attack",
          message: `Stage ${nextStageNum}: ${boss.name} appears!`,
        },
      ],
    }));
  }, [gameState, claimNFT]);
